...
//   ngOnInit() {
//    this.http.get('http://192.168.18.81:5002/getProjectsTree/1').subscribe((response)=>{
...
export class ProjectListComponent{
  ...
  // requestUsers(){
  //   this.http.get('http://192.168.18.81:5002/getprojects/' + this.companyId).subscribe((response)=>{
  ...
  getCompanyName(){
    // if(this.type == "Admin"){
    //   this.http.get('http://192.168.18.81:5002/getCompany/' + this.id).subscribe((response)=>{
    //     this.temp = response as JSON;
    //     this.comapnyName = this.temp[0]['name']
    //   });
    // }
    // if(this.type == "user"){
    this.http.get('http://192.168.18.81:5002/getUser/' + this.id).subscribe((response)=>{
    this.temp = response as JSON
  ...
  delete(index: string | number){
    if(this.projects[index]['project_id']){
      ...
      this.http.get('http://192.168.18.81:5002/deleteProject/' + this.projects[index]['project_id']).subscribe((response)=>{
        ...
        if(this.temp['message'] == 'success'){
          ...
          this.notifierService.notify('error', 'This project has documents containers attached. These need to be deleted before the project can be deleted');
        }
    });
    }
    // location.reload();
  ...
  projectContent(index: string | number){
    return "/app-project-content/" + this.projects[index]['project_id'] + '/' + this.id + '/id'
  }
