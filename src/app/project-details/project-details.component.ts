...
export class ProjectDetailsComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.http.get('http://0.0.0.0:5002/getUser/' + this.projectInfo.proj["customer_id"]).subscribe((response)=>{
      this.users = response as JSON
        this.http.get('http://0.0.0.0:5002/getAdmin/' + this.users[0]["company_id"]).subscribe((response)=>{
    ...
    if(this.projId != ""){
      ...
      this.http.get('http://0.0.0.0:5002/getProject/' + this.projId).subscribe((response)=>{
  ...
  createUpdate(){
    this.http.post('http://0.0.0.0:5002/project', this.projectInfo.proj).subscribe((response)=>{
