...
export class ProjectDetailsComponent implements OnInit, ComponentCanDeactivate {
  ...
  ngOnInit(){
    ...
    this.http.get('http://192.168.18.81:5002/getUser/' + this.userId).subscribe((response)=>{
      ...
      console.log("companyID: " + this.projectInfo.proj["company_id"])
        this.http.get('http://192.168.18.81:5002/getAdmin/' + this.users[0]["company_id"]).subscribe((response)=>{
  ...
  createUpdate(){
    ...
    if(date1 < date2){
      this.http.post('http://192.168.18.81:5002/project', this.projectInfo.proj).subscribe((response)=>{
