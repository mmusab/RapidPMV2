...
export class CompanyListScreenComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    if (token) {
      ...
      this.http.get('http://192.168.1.15:5002/getCompanies').subscribe((response)=>{
  ...
  delete(index: string | number){
    if(this.companies[index]['company_id']){
      ...
      this.http.get('http://192.168.1.15:5002/deleteCompany/' + this.companies[index]['company_id']).subscribe((response)=>{
