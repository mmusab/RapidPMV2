...
export class ArtefactTypeDefaultComponent{
  ...
  //  this.http.get('http://192.168.18.81:5002/getArtefactDefault/' + this.projectId).subscribe((response)=>{
  ...
  onAddType(index: string | number){
    ...
    this.http.post('http://192.168.18.81:5002/type', this.typeDefaults[index]).subscribe((response)=>{
  ...
  requestTypes(){
    this.http.get('http://192.168.18.81:5002/getArtefactDefaults/' + this.projectId).subscribe((response)=>{
      this.typeDefaults = response as JSON
  ...
  deleteRow(index: string | number){
    ...
    if(this.typeDefaults[index]['type_id']){
      this.http.get('http://192.168.18.81:5002/deleteType/' + this.typeDefaults[index]['type_id']).subscribe((response)=>{
        console.log(response)
        location.reload();
    });
    }
  ...
  editField = "";
    ...
    // goToProjects(){
    //   this.http.get('http://192.168.18.81:5002/getAdmin/' + this.signupCompId).subscribe((response)=>{
    ...
    updateAll(){
      ...
      this.http.post('http://192.168.18.81:5002/typeBulkAdd', this.typeDefaults).subscribe((response)=>{
      console.log(response)
      // this.typeId = (response as any)['message'];
      this.notifierService.notify('success', 'Type has been created/updated');
   });
